    ...
    return (
      <Empty className="border">
        <EmptyHeader>
          ...
          <EmptyDescription>New launch guides will appear here when they are published.</EmptyDescription>
        </EmptyHeader>
      </Empty>
    )
  }
        ...
        <li key={article.slug} className="min-w-0">
          <article className="flex h-full flex-col items-start gap-3">
            ...
            <h2 className="font-outfit text-xl leading-snug font-semibold tracking-tight sm:text-2xl">
              <Link href={`/blog/${article.slug}`} className="decoration-border underline-offset-4 hover:underline">
                {article.title}
              </Link>
            </h2>
            ...
            >
              Read guide <ArrowRightIcon className="size-4" aria-hidden="true" />
            </Link>
          </article>
        </li>
      ))}
