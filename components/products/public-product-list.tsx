  ...
  return (
    <ul className="divide-y rounded-xl border px-4 sm:px-5">
      {products.map((product) => (
        <li key={product.id} className="group relative flex cursor-pointer flex-wrap items-center gap-3 rounded-sm py-4 transition-colors hover:bg-muted/30 focus-within:bg-muted/30 sm:flex-nowrap sm:gap-4">
          <Link
          ...
          />
          <Link href={`/products/${product.slug}`} className="relative z-10 flex size-11 shrink-0 items-center justify-center" aria-label={`About ${product.name}`}>
            {product.logoUrl ? (
  ...
  return (
    <nav aria-label="Product pages" className="flex items-center justify-between gap-4">
      <Button asChild variant="outline" size="sm" disabled={page <= 1}>
        <Link href={directoryPageHref(path, page - 1, params)} aria-disabled={page <= 1} tabIndex={page <= 1 ? -1 : undefined} className={page <= 1 ? "pointer-events-none opacity-50" : undefined}>Previous</Link>
      </Button>
      ...
      <Button asChild variant="outline" size="sm" disabled={page >= pageCount}>
        <Link href={directoryPageHref(path, page + 1, params)} aria-disabled={page >= pageCount} tabIndex={page >= pageCount ? -1 : undefined} className={page >= pageCount ? "pointer-events-none opacity-50" : undefined}>Next</Link>
      </Button>
    </nav>
  )
}
