          ...
          {linkGroups.map((group) => (
            <div key={group.title}>
              ...
              <ul className="mt-3 flex flex-col gap-2">
                {group.links.map(([label, href]) => <li key={href}><Link href={href} className="text-sm text-muted-foreground transition-colors hover:text-foreground">{label}</Link></li>)}
              </ul>
            </div>
          ))}
        </nav>
      </SiteContainer>
      <SiteContainer className="border-t py-5 text-xs text-muted-foreground">© {new Date().getUTCFullYear()} ShipBits. Built for people who ship.</SiteContainer>
    </footer>
  )
}
