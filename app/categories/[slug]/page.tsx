...
import { DirectoryPagination, PublicProductList } from "@/components/products/public-product-list"
import {
  DIRECTORY_PAGE_SIZE,
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    description: `Discover published ${category.name.toLowerCase()} products from Filipino builders on ShipBits.`,
    ...
    <main className="py-10 sm:py-14">
      <SiteContainer className="flex flex-col gap-8">
        <nav aria-label="Breadcrumb" className="text-sm text-muted-foreground">
          <Link href="/products" className="hover:underline">Products</Link> <span aria-hidden="true">/</span> <span aria-current="page">{category.name}</span>
        </nav>
        <header className="max-w-2xl">
          ...
          <p className="mt-3 leading-relaxed text-muted-foreground">Browse published {category.name.toLowerCase()} products from Filipino builders. Every listing links to the product and its public ShipBits profile.</p>
        </header>
