...
test("rejects local hostnames, encoded IP forms, credentials, unsafe protocols and ports", () => {
  ...
  assert.equal(
    parsePublicUrl("https://example.com/page#section").toString(),
  ...
  await assert.rejects(
    safeFetchText(
      ...
      fakeNetwork({
        transport: async () => ({
          status: 302,
          headers: { location: "https://elsewhere.example/robots.txt" },
          body: Buffer.alloc(0),
        }),
      })
    ),
...
test("can safely truncate an oversized HTML response when a caller opts in", async () => {
  const response = await safeFetchText(
    "https://example.com/",
    { allowTruncated: true, maxBytes: 4 },
    fakeNetwork({
      transport: async (_url, _address, options) => {
        assert.equal(options.allowTruncated, true)
        return {
          status: 200,
          headers: { "content-type": "text/html" },
...
test("DNS failures expose a useful message, not internal network details", async () => {
  await assert.rejects(
    safeFetchText(
      ...
      fakeNetwork({
        resolve: async () => {
          throw new Error("DNS server 10.10.10.10 failed with internal details")
        },
      })
    ),
    (error: unknown) =>
      ...
      !error.message.includes("10.10.10.10")
  )
})
