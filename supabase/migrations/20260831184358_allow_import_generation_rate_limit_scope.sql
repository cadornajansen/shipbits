...
alter table public.public_request_limits
  add constraint public_request_limits_scope_check check (scope in (
    ...
    'listing-payment', 'directory-campaign', 'directory-payment',
    'import-generation', 'upvote-payment'
  ));
...
begin
  if p_scope not in ('seo-checker', 'distribution-finder', 'newsletter', 'autocomplete', 'listing-payment', 'directory-campaign', 'directory-payment', 'import-generation', 'upvote-payment')
