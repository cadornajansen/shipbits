create function public.valid_product_tags(value text[])
returns boolean
language sql
immutable
set search_path = ''
as $$
  select cardinality(value) <= 5
    ...
    and coalesce((select bool_and(tag = btrim(tag) and tag <> '' and char_length(tag) <= 30)
      from unnest(value) as tag), true);
$$;

alter table public.products
  add column tags text[] not null default '{}',
  add constraint products_tags_check check (public.valid_product_tags(tags));

alter table public.listing_submissions
  add column tags text[] not null default '{}',
  add constraint listing_submissions_tags_check check (public.valid_product_tags(tags));
...
begin
  ...
  if not found then raise exception 'Listing payment not found'; end if;
  if payment_row.status = 'paid' then return null; end if;
  if payment_row.status <> 'pending' then raise exception 'Listing payment is not pending'; end if;
  ...
  where submission_id = submission_row.id and type in ('logo', 'cover');
  ...
  if submission_row.status = 'submitted' then
    ...
    set paid_at = now(), provider_event_id = coalesce(provider_event_id, p_provider_event_id),
      provider_payment_id = coalesce(provider_payment_id, p_provider_payment_id), status = 'paid'
  ...
  insert into public.products (
    ...
    short_description, slug, tagline, tags, website_url
  ) values (
    ...
    p_product_slug, submission_row.tagline, submission_row.tags, submission_row.website_url
